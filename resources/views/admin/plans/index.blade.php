...
@section('title', __('app.dashboard-plan'))
    ...
    <div class="main-content">
        <div class="page-content">
            <div class="container-fluid">
                <!-- start page title -->
                @include('layouts.admin.title', ['title' => __('app.plans')])
                <!-- end page title -->
                                ...
                                <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                                    ...
                                    @foreach ($plans as $key => $plan)
                                        <tr>
                                            ...
                                            <td class="text-center col-2">{{ $plan['name'] }}</td>
                                            <td class="text-center col-2">{{ $plan['description'] }}</td>
                                            <td class="text-center col-2">
                                                {{--                                                    @if(AdminPermission('edit plan'))--}}
                                                <a class="btn btn-outline-warning btn-sm edit" title="Edit"
                                                   href="{{ route('admin.plans.edit', $plan['id']) }}">
                                                    <i class="fas fa-pencil-alt" title="Edit"></i>
                                                </a>
                                                {{--                                                    @endif--}}
                                                {{--                                                    @if(AdminPermission('delete plan'))--}}
                                                <form method="post"
